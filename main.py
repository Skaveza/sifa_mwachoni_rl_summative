...
def save_gif(env, filename="results/random_agent.gif", max_steps=200, fps=15):
    """Record random agent as GIF using Renderer, excluding cumulative reward."""
    ...
    if frames:
        ...
        print(f"Saved Renderer GIF to {filename}")
...
def record_agent_performance(
    ...
    """Record agent performance as MP4 using Renderer."""
...
def generate_demonstrations(env) -> Dict[str, List[Dict]]:
    """Generate visual demonstrations for random and trained agents."""
    os.makedirs("results", exist_ok=True)
    performance_data = {}
    
    # Random agent baseline (no cumulative reward in visualization)
